...
export default function CreatePOPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    state: 'confirmed', // Auto-confirm for speed as per requirements
  ...
  const updateLine = (index: number, field: string, value: any) => {
    ...
    if (field === 'product') {
      ...
      if (p) {
          // Use 'purchase_price' (Cost Price) for POs
          newLines[index].price_unit = p.purchase_price || p.price || 0; 
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.post('/finance/purchase-orders/', formData);
    ...
    <div className="min-h-screen bg-[#0f172a] p-8 text-white">
      <div className="max-w-4xl mx-auto mb-8 flex items-center justify-between">
        <div className="flex items-center gap-4">
            ...
            <ShoppingCart className="text-blue-400" /> New Purchase Order
            </h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto bg-[#1e293b] border border-slate-700 rounded-xl p-8 shadow-xl">
        ...
        <div className="grid grid-cols-2 gap-8 mb-8 border-b border-slate-700 pb-8">
          <div>
            <label className="block text-slate-400 text-sm font-bold mb-2">Vendor</label>
            <select 
              className="w-full bg-[#0f172a] border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:border-blue-500 transition-colors"
              value={formData.partner}
              onChange={(e) => setFormData({...formData, partner: e.target.value})}
              required
            >
              <option value="" className="text-slate-500">Select Vendor...</option>
          ...
          <div>
            ...
            <input 
              type="date" 
              className="w-full bg-[#0f172a] border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:border-blue-500"
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-4">
             <h3 className="font-bold text-slate-300 text-lg">Product Lines</h3>
          </div>
          ...
          <div className="space-y-3">
            {formData.lines.map((line, index) => (
                ...
                <div className="flex-1">
                    ...
                    <select 
                    className="w-full bg-transparent border-b border-slate-600 py-2 text-sm text-white focus:outline-none focus:border-blue-500"
                    value={line.product}
                ...
                <div className="w-24">
                    ...
                    <input type="number" className="w-full bg-transparent border-b border-slate-600 py-2 text-sm text-white focus:outline-none focus:border-blue-500 text-center"
                ...
                <div className="w-32">
                    ...
                    <div className="relative">
                        ...
                        <input type="number" className="w-full bg-transparent border-b border-slate-600 py-2 pl-4 text-sm text-white focus:outline-none focus:border-blue-500"
          ...
          <button type="button" onClick={addLine} className="text-blue-400 hover:text-blue-300 text-sm flex items-center font-bold mt-4 px-2 py-1 rounded border border-blue-500/30 bg-blue-500/10">
        ...
        <div className="flex justify-end pt-6 border-t border-slate-700 gap-4">
          ...
          <button type="submit" className="px-8 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg font-bold shadow-lg shadow-blue-500/20 flex items-center transition-all">
            <Save size={18} className="mr-2" /> Confirm Order
          </button>
        </div>
      </form>
    </div>
