...
export default function CreatePOPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    state: 'confirmed', // Auto-confirm for speed
    ...
    <div className="min-h-screen bg-[#0f172a] p-8 text-white">
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto bg-[#1e293b] border border-slate-700 rounded-xl p-8 shadow-xl">
        ...
        <div className="space-y-4 mb-8">
          <h3 className="font-bold text-slate-300">Products</h3>
          {formData.lines.map((line, index) => (
            <div key={index} className="flex gap-4 items-end bg-[#0f172a] p-3 rounded border border-slate-800">
              ...
              <div className="w-24">
                ...
                <input type="number" className="w-full bg-[#1e293b] border border-slate-600 rounded p-2 text-sm text-white"
          ...
          <button type="button" onClick={addLine} className="text-blue-400 text-sm flex items-center font-bold mt-2">
            <Plus size={16} className="mr-1" /> Add Product
          </button>
        </div>

        <div className="flex justify-end pt-6 border-t border-slate-700">
          <button type="submit" className="px-8 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg font-bold shadow-lg flex items-center">
